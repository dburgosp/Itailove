<?php
  ...
  if (
    ...
    && flo_data($section_data, "flo-block__merge-with-header", false)
  ) {
  ...
  if (isset($section_data_onready) && $section_data_onready !== "" ) {
    $section_data_onready = "data-onready='".$section_data_onready."'";
  } else {
    $section_data_onready = "";
  }
?>
@include('core.style', [
...
@endif
<div  {{$flo_block_id}} class='flo-block {{$block_classes or ""}} {{$flo_block_classes}} {{$merge_with_header ? "flo-block--merged-with-header" : "" }}  {{ $block_class }} @yield('block_classes') @section('block_classes') @overwrite ' data-id="{{ $block_id }}" {{$section_data_onready}}>
