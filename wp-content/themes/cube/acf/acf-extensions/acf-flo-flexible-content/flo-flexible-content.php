...
<?php
  ...
  function input_admin_enqueue_scripts() {
    ...
    wp_register_script( 'acf-flo-flexible-content', "{$dir}flo-flexible-content.js", array('acf-input'), acf_get_setting('version'));
...
function flo_add_layout_block($layout_block = '', $page_url = '', $screen_post_type = '') {
	...
	if($page_url != ''){
		...
		if( isset($url_args) && $refferer_is_blog_post ){
			...
			// $single_posts_fields = get_field( 'flo-l2-p_layout', 'options' );
			// foreach ($single_posts_fields as $k => $v) {
			// 	// for the Title 1 block, we get this kind of data: https://pastebin.com/2xGK70ZN
			// 	if( isset($v['acf_fc_layout']) && $v['acf_fc_layout'] == $layout_block ) {
			// 		$custom_block_val = $v;
			// 		wp_mail('guzun.allex@gmail.com', 'testtik - '.$layout_block , var_export($v,true) );		
			// 	}
			// }

			

			//$single_posts_fields = get_field_object( $single_posts_fields_key, 'options' );
			// if(isset($single_posts_fields['sub_fields'][0])) {
			// 	foreach ($single_posts_fields['sub_fields'][0]['layouts'] as $k => $v) {
			// 		if($v['name'] == $layout_block ) {
			// 			wp_mail('guzun.allex@gmail.com', 'testtik - '.$layout_block , var_export($v,true) );		
			// 			$custom_block_val = $v['sub_fields'];
			// 		}
			// 	}	
			// }
...
function flo_add_template_blocks() {
	...
	foreach ($template_blocks as $key => $template_block) {
		$layout_blocks[$template_block.'_'.$i] = flo_add_layout_block( $layout_block = $template_block, $page_url = $_POST['page_url'], $screen_post_type );
		$i++;
	}
